...
Please refer to `config.py` for the exact directories where the patient and trial JSON files need be present for matchminer-admin to process them.
...
# This option reads the trial_status.csv and based on trial's last updated time and matchmiiner admin's last run time,
# decides if a trial in trial_status.csv needs to be processed or not.
...
python trial.py trial update --protocol_no 2024060101 --updated_trial_file /path/to/updated_trial.json
...
## 5. Deployment as a service on Linux
...
3.  **View service logs:**
    ```bash
    sudo journalctl -u matchminer-admin.service -f -n 50
    ```
...
2. **`../nct2ctml/sync_trials.sh`** - Syncs trials from clinicaltrials.gov and saves trial CTML files locally
3. **`start_processor.sh`** - Processes patient and trial JSON files by inserting/updating data into matchminer DB
